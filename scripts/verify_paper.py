...
# A reference must carry something a reader can follow; an entry with none is
# where a fabricated citation hides (references.md / citations.md).
LOCATOR = re.compile(r"(?im)^\s*(?:doi|url|eprint|isbn|howpublished)\s*=|\\url\{")
AI_WORDS = re.compile(
...
def main() -> int:
    ...
    for f in bib_files:
        ...
        bib_keys.update(BIBKEY.findall(btext))
        for block in re.split(r"(?m)^(?=@)", btext):
            km = BIBKEY.search(block)
            if km and not LOCATOR.search(block):
                soft.append(f"bib entry {km.group(1)} has no locator (doi/url/eprint/isbn)")
